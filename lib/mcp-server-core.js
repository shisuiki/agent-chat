...
function detectAgentName() {
  ...
  try {
    ...
    if (tty) {
      ...
      for (const line of paneLines.split('\n')) {
        ...
        if (paneTty === expected && session) return { name: session, source: 'process-tty' };
      }
    }
  } catch {}
  ...
  try {
    ...
    if (fromClient) return { name: fromClient, source: 'tmux-client' };
  } catch {}
...
function hasAgentChatMembershipEnv() {
  return Boolean(
    rawEnv('AGENTCHAT_AGENT_STATE_DIR')
    || rawEnv('AGENT_CHAT_RUNTIME_DIR')
    || rawEnv('AGENTCHAT_HOMEDIR')
  );
}

const AGENT_DETECTION = detectAgentName();
const AGENT_NAME = AGENT_DETECTION?.name || 'unbound';
const AGENT_NAME_SOURCE = AGENT_DETECTION?.source || 'none';
const MCP_AGENT_BOUND = AGENT_NAME_SOURCE === 'env' && hasAgentChatMembershipEnv();
...
const MEDIA_FETCH_CACHE_DIR = MCP_AGENT_BOUND ? resolveMediaFetchCacheDir(AGENT_NAME) : null;
if (MEDIA_FETCH_CACHE_DIR) mkdirSync(MEDIA_FETCH_CACHE_DIR, { recursive: true });
...
function nonMemberReason() {
  if (AGENT_NAME_SOURCE !== 'env') {
    return `agent identity was ${AGENT_NAME_SOURCE === 'none' ? 'not found' : `inferred from ${AGENT_NAME_SOURCE}`} instead of explicit AGENT_NAME from agent-up`;
  }
  return 'AGENT_NAME is set but no Agent Chat membership env was provided';
}

function notAgentChatMemberError() {
  return err([
    'Signal: not_agentchat_member',
    'This global Agent Chat MCP server is not bound to an Agent Chat-managed runtime.',
    `Detected name: ${AGENT_NAME_SOURCE === 'none' ? '(none)' : AGENT_NAME}`,
    `Reason: ${nonMemberReason()}.`,
    'Start the agent with `agentchat up` or provide per-agent MCP env including AGENT_NAME plus AGENTCHAT_AGENT_STATE_DIR, AGENT_CHAT_RUNTIME_DIR, or AGENTCHAT_HOMEDIR.',
  ].join('\n'));
}

function memberTool(handler) {
  return async (args) => {
    if (!MCP_AGENT_BOUND) return notAgentChatMemberError();
    return handler(args);
  };
}
...
if (MCP_AGENT_BOUND) {
  ...
  process.stderr.write(`[mcp] not bound to Agent Chat member (${nonMemberReason()}); tools will return not_agentchat_member and no heartbeat will be sent\n`);
}

// 1. whoami — returns concise identity, groups, and known agent names
server.tool('whoami', 'Returns your agent identity, role, and groups', {}, memberTool(async () => {
  try {
...
server.tool(
  ...
  memberTool(async ({ to, summary, full, attachments, type, priority, reply_to, schema }) => {
    try {
...
server.tool(
  ...
  memberTool(async ({ group, summary, full, attachments, type, priority, mentions, reply_to, schema }) => {
    try {
      const stagedAttachments = await stageAttachments(attachments);
      const data = await api('POST', '/api/messages', {
...
server.tool(
  ...
  memberTool(async ({ group, limit, unread_limit, read_all }) => {
    try {
...
server.tool(
  ...
  memberTool(async ({ assignee, status, priority, label, limit, offset }) => {
    try {
      const params = new URLSearchParams();
...
server.tool(
  ...
  memberTool(async ({ id, status, waiting_reason, waiting_until }) => {
    try {
      const body = { status };
...
server.tool(
  ...
  memberTool(async ({ id, text: commentText }) => {
    try {
...
server.tool(
  ...
  memberTool(async ({ id, heartbeat, waiting_reason, waiting_until }) => {
    try {
      const body = {};
...
// ── PID file ──────────────────────────────────────────────────────────
const MCP_PID_FILE = (() => {
  if (!MCP_AGENT_BOUND) return null;
